...
export function WorkCard({ work, locale }: Props) {
  ...
  return (
    <Tooltip>
      <TooltipTrigger
        render={
          <motion.article
            ...
            className="group relative w-full rounded-lg border border-border bg-card/50 p-4 text-left transition-colors duration-300 outline-none hover:border-brand/40 hover:bg-card hover:shadow-md focus-visible:border-brand/40 focus-visible:bg-card"
          />
        }
      >
      ...
      <TooltipContent>{work.description[locale as "ja" | "en"]}</TooltipContent>
    </Tooltip>
  );
}
